...
// Generated from Bluetooth SIG Assigned Numbers Repository:
// https://bitbucket.org/bluetooth-SIG/public/src/HEAD/assigned_numbers/company_identifiers/company_identifiers.yaml
// Regenerate when Bluetooth SIG updates Company Identifiers.
enum ManufacturerDirectory {
    static let companyNames: [UInt16: String] = Dictionary(uniqueKeysWithValues: entries)

    private static let entries: [(UInt16, String)] = [
        (0x0000, "Ericsson AB"),
    ...
    static func displayName(for companyID: UInt16) -> String {
        let hex = String(format: "0x%04X", Int(companyID))
        if let name = name(for: companyID) {
            return "\(name) (\(hex))"
        }
        return "未知厂商 (\(hex))"
    }
